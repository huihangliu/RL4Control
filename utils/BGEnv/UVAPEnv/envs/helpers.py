...
LongAction = namedtuple(
    "LongAction",
    ["basal_rate_incr", "correction_factor_incr", "carbohydrate_ratio_incr"],
)
OLStateBins = namedtuple("OLStateBins", ["hypo_bins", "hyper_bins"])
OLActionBins = namedtuple(
    "OLActionBins", ["basal_rate_bins", "carbohydrate_ratio_bins"]
)
...
class Seed:
    # tuple is not mutable, so we need to use class
    def __init__(self, sensor_seed, scenario_seed, numpy_seed):
